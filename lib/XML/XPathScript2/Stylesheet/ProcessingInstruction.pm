package XML::XPathScript2::Stylesheet::ProcessingInstruction;
...
has rename => ( is => 'rw', predicate => 'has_rename' );
...
sub transform {
    ...
    if ( $self->showtag ) {
        $output .= $self->has_rename 
                    ? '<'.$self->rename.'>'
                    : '<?' . $node->name . ' '
                    ;
    }

    my $content  = $self->action < 0 ? undef
                 : $self->replace    ? $self->render( $self->replace, $node, $args )
                 :                     $node->content
                 ;
